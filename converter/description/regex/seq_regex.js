...
var whole_node = /^\s*(boundary|entity|actor|control|participant)\s+\"(.+)\"\s+as\s+([^\s#]+)\s*(#red)?\s*$/gm
var short_node = /^\s*(boundary|entity|actor|control|participant)\s+([^\s#]+)\s*(#red)?\s*$/gm
